...
std::unique_ptr<Fuser> createFuser(const std::string base_path, const std::string dataset_name);

///@brief A class for loading aug icl nuim data
class DataLoader : public RgbdDataLoaderInterface {
 public:
  /// Constructors not intended to be called directly, use factor
  /// DataLoader::create();
  DataLoader(const std::string& base_path, const std::string& dataset_name, bool multithreaded = true);
  virtual ~DataLoader() = default;

  /// Builds a DatasetLoader
  ///@param base_path Path to the augmented icl-nuim dataset sequence base folder.
  ///@param base_path Path to the augmented icl-nuim dataset sequence base folder.
